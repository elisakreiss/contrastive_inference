...
She suggests that these objects differ in how likely a speaker is to produce the color modifier for the object in isolation: in the absence of a contrast, a yellow banana is usually just called \textit{the banana} while a yellow cup is often called \textit{the yellow cup}, which \citeA{Sedivy:2003} calls these objects' \emph{default descriptions}. Only in the cases where the modifier is not part of the default description, she argues, is the observation surprising and can be interpreted by the listener as a contrastive signal that will elicit the contrastive inference.
