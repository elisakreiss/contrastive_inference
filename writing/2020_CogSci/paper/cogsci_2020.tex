...
\figref{prod-results} shows the proportion of color modifier mentions for the target and competitor in each condition. %\footnote{Note that some data is duplicated in the conditions where the contrast is absent (as described in \textit{Design})}. 
When a contrast to the target was present (e.g., another banana),  participants almost always mentioned the color modifier to fully disambiguate the two items (see the upper row in \figref{prod-results}), especially when the target was atypical. When the contrast was absent (see the lower row in \figref{prod-results}), participants were more likely to include a color modifier when referring to an atypical target than a typical one. Independent of contrast, speakers were more likely to include the color modifier for an atypical color competitor over a typical one. \jd{no mixed effects models at all anymore??}
...
\begin{figure}
	...
	\end{center}
\caption{Correlation of RSA model predictions (x axis) and the empirically elicited proportion of target selections. The dashed lines mark the chance level for target versus competitor selections. Data points are color coded according to the conditions in \figref{modelcompr-results}.} 
