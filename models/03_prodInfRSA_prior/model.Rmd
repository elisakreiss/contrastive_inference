...
## Flat prior (no empirical preadj data used)
...
df_full %>% 
  select(obj_in_display, RSA_noprior, emp, condition, obj_typ) %>% 
  filter(obj_in_display == "target") %>% 
  ...
  scale_fill_manual(values=c("#808080", "#657590")) +
...
ggsave(here("analyses","full_analysis","graphs","rsa_noprior_bar.png"), height=4, width=6, bg="transparent")
```
...
df_full %>%  
  ...
  ggplot(., aes(x=meanRSA, y=meanemp, color=condition)) +
...
df_full %>%  
  ...
  ggplot(., aes(x=meanRSA, y=meanemp, color=condition)) +
    geom_point(data=filter(df_full, obj_in_display=="target"),
               aes(x=RSA_weirdprior, y=emp_weirdprior, color=condition),
               alpha=0.2) +
    geom_errorbar(aes(ymin = emp_CI_low, ymax = emp_CI_high),
                  width=0) + 
    geom_errorbarh(aes(xmin = RSA_CI_low, xmax = RSA_CI_high),
