...
```{r HitCorrect check, eval=FALSE, fig.height=1.5, fig.width=4, include=FALSE}
...
mean_time_nooutliers = round(
  mean(
    df_subj[df_subj$timeSpent <= (mean_time + sd(df_subj$timeSpent)),]$timeSpent
  ), digits = 1)
p_time = ggplot(df_subj,aes(x=timeSpent)) +
  ...
  scale_x_continuous(breaks=c(mean_time_nooutliers,mean_time,floor(0),floor(10),floor(25),floor(20))) +
  xlab("time spent")
