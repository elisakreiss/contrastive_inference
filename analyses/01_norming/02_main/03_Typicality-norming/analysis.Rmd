...
df_plot = df %>% 
  ...
  separate(question, c("color", "item"), sep = "_", remove = FALSE) %>% 
  # identify presumably color diagnostic object
  mutate(cd = ifelse(item %in% cd_list, T, F))

```

```{r typicality ratings cd, echo=FALSE, fig.height=6, fig.width=8, message=FALSE, warning=FALSE}

df_cd = df_plot %>% 
  filter(cd == T) %>% 
  group_by(item, color) %>% 
  mutate(mean_y = mean(slider_val)) %>% 
  ungroup()

ggplot(df_cd, aes(x=reorder(question, -slider_val), y=slider_val)) +
...
```{r typicality ratings noncd, echo=FALSE, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
