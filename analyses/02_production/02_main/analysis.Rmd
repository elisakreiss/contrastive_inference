...
df_import1 = read_csv(here::here("data","02_production","02_main","data1.csv")) %>%
  # exclude test
...
df_import = read_csv(here::here("data","02_production","02_main","data2.csv")) %>%
  # exclude test
  # weird filter property will exclude NAs
  filter(is.na(comments) | !str_detect(comments,"TEST")) %>% 
  filter(!str_detect(conversation, "elisakreiss")) %>% 
...
# number of participants before exclusion (68; 34)
length(unique(df_import$anon_worker_id))
# number of participants after exclusion (29)
...
df_main = df %>% 
  ...
  select(intended_target, condition, speaker_chat, trial_number, anon_worker_id, target_item, targetType, comp_item, selected_image) %>% 
...
df_rsa_target = df_model_prereg %>%
  ...
  select(uttProb, condition, intended_target)
...
f_prereg = ColMentioned ~ contrast_present*c_target_typ*c_comp_typ + (1+contrast_present+c_target_typ+c_comp_typ|anon_worker_id) + (1+contrast_present+c_target_typ+c_comp_typ|targetType)
