...
df_import1 = read_csv(here::here("data","02_production","02_main","01_refgame","data1.csv")) %>%
  # exclude test
...
df_import2 = read_csv(here::here("data","02_production","02_main","02_refgame_followup","data.csv")) %>%
  # exclude test
  # weird filter property will exclude NAs
  filter(is.na(comments) | !str_detect(comments,"TEST")) %>%
  filter(!str_detect(conversation, "elisakreiss"))

df_import = read_csv(here::here("data","02_production","02_main","01_refgame","data2.csv")) %>%
  # exclude test
  # weird filter property will exclude NAs
  filter(is.na(comments) | !str_detect(comments,"TEST")) %>% 
  filter(!str_detect(conversation, "elisakreiss")) %>% 
...
X participant pairs were recruited.
...
# gender: 128 female; 150 male; 2 other; 2 chose not to respond
df_gender = df_import %>%
  distinct(anon_worker_id, gender)
table(df_gender$gender)

# mean age: 37.1 years; 12 chose not to respond
df_age =  df_import %>%
  distinct(anon_worker_id, age) %>% 
...
df_time = df_import %>%
  ...
  mutate_at(vars(endTime, startTime), funs(./60000)) %>% 
  select(startTime, startDate, endTime, time, timeSpent, speaker_chat) %>% 
  view()
  distinct(anon_worker_id, time) %>% 
  mutate_at(vars(time), funs(./60000))
df_time
...
types = c("banana|banaa|bananna|nana|banan|bannaa|ban|broccoli|broc|carrot|carro|carot|cararot|c arrot|corn|tomato|tom|tamayo|egg|strawberry|straw|str|berry|lettuce|letc|cabbage|cabb|cabage|swan|goose|duck|pumpkin|pump|punmkn|pum")

df_cleanedutts = df_preclean %>% 
...
df_clean = df_cleanedutts %>%
  filter(!(anon_worker_id %in% taboo_player_list))

# number of participants before exclusion (282; 141)
length(unique(df_import$anon_worker_id))
# number of participants after exclusion (112)
...
# ggsave(here("writing", "2020_CogSci", "paper", "graphs", "prod-bycondition-contrast.pdf"), width = 8, height = 3)
```
...
# merge with typicality ratings data for target and competitor
df_main_typ = df_main %>% 
...
df_model_prereg_targetonly = df_main_typ %>% 
  filter(intended_target == "target" ) %>%
  mutate(wrong_selection = !str_detect(selected_image, target_item)) %>% 
  filter(!wrong_selection) %>% 
  filter(UtteranceCat=="colorType" | UtteranceCat=="typeOnly") %>% 
...
df_model_prereg_targetcomp = df_main %>% 
...
# prereg
# modifier mention ~ contrast presence*typicality of target*typicality of competitor + (1+contrast presence*typicality of target|participant) + (1+contrast presence*typicality of target*typicality of competitor|item)
# without target typ and contrast interaction:
# modifier mention ~ contrast presence*typicality of competitor + typicality of competitor*typicality of target + (1+contrast presence+typicality of target|participant) + (1+contrast presence*typicality of competitor + typicality of competitor*typicality of target|item)

m = glmer(ColMentioned ~ c_contrast*c_comp_typ_bin + c_comp_typ_bin*c_target_typ_bin + (1+c_contrast+c_target_typ_bin|anon_worker_id)  + (1+c_contrast*c_comp_typ_bin + c_comp_typ_bin*c_target_typ_bin|targetType), data = df_model_prereg, family = "binomial")


m = glmer(ColMentioned ~ c_contrast*c_comp_typ_bin + c_comp_typ_bin*c_target_typ_bin + (1|anon_worker_id), data = df_model_prereg, family = "binomial")
summary(m)

# Very similar to Cogsci paper results but intercept Rhat is 1.01
model_prereg = brm(
  formula = ColMentioned ~ c_contrast*c_comp_typ_bin + c_comp_typ_bin*c_target_typ_bin + (1|anon_worker_id) + (1|targetType),
...
model_prereg = brm(
  formula = ColMentioned ~ c_contrast*c_comp_typ_bin + c_comp_typ_bin*c_target_typ_bin + (1|anon_worker_id),
  data = df_model_prereg,
  seed = 1702,
  family = 'bernoulli' # or bernoulli?
)
...
# REPORTED IN COGSCI PAPER in footnote
model_prereg = brm(
  formula = ColMentioned ~ c_contrast*c_nonreferent_typ_bin + c_nonreferent_typ_bin*c_referent_typ_bin + (1|anon_worker_id),
  data = df_model_prereg_targetcomp,
  seed = 1702,
  family = 'bernoulli',
  iter=4000
)
