...
# 3472 (97.8% of all critical messages only contain a single speaker message)
df_singleTurnUtts %>%
  filter(intended_target == "target" | intended_target == "firstDistractor") %>%
  nrow()
...
# # 6264 (96%)
# nrow(df)
# # 3327 (XX.X% have correct selections on the critical trials)
# df %>%
...
df_main = df %>% 
  # these would otherwise go into OtherModMention 
...
table(filter(df_main, intended_target == "target" | intended_target == "competitor")$UtteranceCat)
...
df_nocontrdupl = df_main %>% 
  ...
  # select modified columns only to avoid confusion
...
```{r write data, eval=FALSE, include=FALSE}
...
# ggsave("tan_condition_empcriticalspeaker.png", height=2.6, width=2.8, bg="transparent")
```
...
f = ColMentioned ~ c_contrast + c_nonreferent_typ_bin + c_referent_typ_bin + (1|anon_worker_id)
m = lmer(f, df_stats)
# main effect of intercept, contrast, referent_typ_bin
summary(m)
