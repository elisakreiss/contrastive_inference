...
df_import = read_csv(here("data","03_comprehension","02_main","0304_IDTpriormanipulation","data.csv")) %>%
  # exclude test
...
df_clean = df_import %>% 
  ...
  # filter(anon_worker_id!="051134074799198")
  filter(anon_worker_id!="073292564463958" & anon_worker_id!="116094209453297" & anon_worker_id!="182787438180499" & anon_worker_id!="182787438180499" & anon_worker_id!="193929304876402" & anon_worker_id!="260916623895607" & anon_worker_id!="408489803795468" & anon_worker_id!="415823359368771" & anon_worker_id!="484135221025167" & anon_worker_id!="511086193615292" & anon_worker_id!="618334277125045" & anon_worker_id!="751451021316546" & anon_worker_id!="756928657241156")
  # 67, 4012
...
table(df_clean$betwsubj, df_clean$prior)/55
```
...
```{r}
df %>% 
  select(anon_worker_id, failed_question, prior) %>% 
  distinct() %>% 
  ggplot(., aes(x=failed_question)) +
    facet_wrap(~prior) +
    geom_bar(stat="count")
```
...
df_main_unique = df %>% 
  ...
  select(prior,trial_type,refObject,context_id,condition,utterance,utterance_cat,
...
df_main_prop %>% 
  ...
  write_csv(., "emp_prevsel_prior.csv", col_names = TRUE)
```
...
#### Typicality target bias

```{r}
df_prior_wfillers_unique %>% 
  mutate(refObj_typ = case_when(
    refObject == "target" ~ target_typ_bin,
    refObject == "comp" ~ comp_typ_bin,
    refObject == "contrast" ~ contrast_typ_bin,
    refObject == "distractor" ~ distractor_typ_bin,
    TRUE ~ "FIRE"
  )) %>%
  mutate_at(vars(refObj_typ), funs(ifelse(. == "atypical", 1, 0))) %>%
  select(refObject, refObj_typ, trial_type, prior) %>% 
  ggplot(., aes(x=prior, y=refObj_typ)) +
    # facet_wrap(~refObject) +
...
df_prior_wfillers_unique %>% 
  ...
  mutate_at(vars(refObj_typ), funs(ifelse(. == "atypical", 1, 0))) %>%
  select(refObject, refObj_typ, trial_type, prior) %>% 
  ggplot(., aes(x=trial_type, y=refObj_typ)) +
    # facet_wrap(~refObject) +
    stat_summary(fun.y = "mean", 
               geom = "bar",
               width = 0.7,
...
df_prior_wfillers_unique %>% 
  ...
  # mutate_at(vars(refObj_typ), funs(ifelse(. == "atypical", 1, 0))) %>%
  select(refObject, refObj_typ, trial_type, prior) %>% 
  ggplot(., aes(x=trial_type, fill=refObj_typ)) +
    facet_wrap(~refObject) +
...
```{r}
df_prior_wfillers_unique %>% 
  select(utterance_cat, trial_type, prior, refObject) %>% 
  ggplot(., aes(x=utterance_cat)) +
    geom_bar(stat="count")

df_prior_wfillers_unique %>% 
  mutate_at(vars(utterance_cat), funs(ifelse(. == "modified", 1, 0))) %>%
  select(refObject, trial_type, prior, utterance_cat) %>% 
  ggplot(., aes(x=prior, y=utterance_cat)) +
    # facet_wrap(~refObject) +
    stat_summary(fun.y = "mean", 
               geom = "bar",
               width = 0.7,
...
df_prior_wfillers_unique %>% 
  select(utterance_cat, trial_type, prior, refObject) %>% 
  ggplot(., aes(x=utterance_cat, fill=refObject)) +
...
df_byitemprior = df_prior_wfillers_unique %>%
  ...
  select(clickedType, target, comp, contrast, distractor, prior) %>% 
  gather(obj_fct, item, -clickedType, -prior) %>% 
...
```{r after adj cogsci paper cond, eval=FALSE, fig.height=4, fig.width=7, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
df_byitemadj = df_adj %>%
  ...
  select(clickedType, target, comp, anon_worker_id, trial_number, prior) %>% 
  distinct() %>% 
...
```{r rsa}
write_csv(df_prior_vs_adj, here("models", "03_prodInfRSA_prior", "data", "emp_comprehension_data.csv"))
```
...
df_model_prevclick =  df %>% 
  ...
  filter(trial_type == "critical") %>% 
  # select columns relevant for main trials
  select(prior,target,comp,contrast,distractor,selectedItem_prior,selectedItem1,
  ...
  mutate(prior_cond = ifelse(prior == "normal", 0, 1)) %>% 
  mutate(c_prior_cond = prior_cond-mean(prior_cond)) %>%
...
# click to target ~ contrast presence*typicality of target*typicality of competitor*prior + preadj selection + (1+contrast*typicality of target*prior|participant) +(1+contrast*typicality of competitor*prior|target) + (1+contrast*typicality of target*prior|competitor)
f_new1 = click_to_target ~ c_contrast*c_target_typ_bin*c_comp_typ_bin*c_prior_cond + c_prev_click + (1+c_contrast*c_target_typ_bin*c_prior_cond|anon_worker_id) + (1+c_contrast*c_comp_typ_bin*c_prior_cond|target) + (1+c_contrast*c_target_typ_bin*c_prior_cond|comp)
