...
ggplot(df_prior_wf_contrast, aes(x=obj_in_display_diff, y=clicked, fill=obj_in_display_diff, alpha=trial_half)) +
  facet_wrap(vars(contrast_present), scales = "free_x") +
  stat_summary(fun.y = "mean", 
               geom = "bar",
               width = 0.7,
...
TODO: (for whole document) differentiate visualization between halfs and prior vs. after adj; maybe halfs through different patterns and prior vs after adj through alpha
...
When target and competitor have the same typicality and there is a contrast, there is a preference for the object with the contrast (i.e., the target). The prior is flat here. This simply reflects the overall pattern.
...
When contrast is absent and target and competitor differ in typicality, there is a preference for the atypically colored objects. This simply reflects the overall pattern.

TODO: summarize this and previous result just verbally under the overall patterns for contrast and atypicality.
...
formula_predprior_aan = prior_click ~ c_pos + (1+c_pos|anon_worker_id) + (1|target)
...
plot_aan = 
  model_predprior_aan %>% 
  as_tibble() %>% 
  select("b_Intercept", "b_c_pos") %>% 
  gather(key = "parameter", value = "posterior") %>% 
  mutate_at(vars(parameter), funs(str_replace_all(.,"b_",""))) %>%
  mutate_at(vars(parameter), funs(str_replace_all(.,"\\.",":"))) %>%
  mutate(parameter = as.factor(parameter)) %>%
  mutate(parameter = factor(parameter, levels = c("Intercept", "c_pos"))) %>%
  ggplot(aes(x = posterior)) + 
    geom_density(fill = "grey") +
    facet_wrap(~ parameter, scales = "free") +
  ylab("density\n") +
  ...
  geom_segment(
    ...
    data = fixef(model_predprior_aan) %>% as_tibble() %>%
      mutate(parameter = c("Intercept", "c_pos")) %>%
      mutate(parameter = as.factor(parameter)) %>%
      mutate(parameter = factor(parameter, levels = c("Intercept", "c_pos"))) %>%
      rename(low = Q2.5, high = Q97.5))
...
ggplot(df_comppos_aan, aes(x=position, y=comp_pos, fill=position)) +
...
ggplot(df_comppos_aan, aes(x=position, y=target_pos, fill=position)) +
  facet_wrap(~condition) +
  stat_summary(fun.y = "mean", 
               geom = "bar") +
  stat_summary(fun.data = "mean_cl_boot",
