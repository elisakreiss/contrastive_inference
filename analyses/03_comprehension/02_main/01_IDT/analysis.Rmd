...
# TODO: what should the data exclusion be here?
# 1) if hit was reportedly done incorrectly (i.e., HitCorrect)
...
# TODO: check that typicality visualizations are valid
# TODO: include the statistical analyses
...
df_adj %>% 
  filter(clicked & (clickedType == "contrast/distractor" | clickedType == "distractor")) %>% 
  nrow()
...
(This is only possible with the data set where the color competitor is typical. -- but also compare the aap to the aan context -- what is the difference?)
...
Do the results look different when target and color competitor are atypical?
Are typicality considerations relevant for the contrastive inference effects (or whatever this is here)?
...
TODO: look over the following three plots
...
```{r after noun errors, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

df_noun %>% 
  filter(clicked & clickedType != "target") %>% 
  nrow()

ggplot(df_noun, aes(x=obj_in_display, y=clicked, fill=obj_in_display)) +
  stat_summary(fun.y = "mean", 
               geom = "bar") +
  stat_summary(fun.data = "mean_cl_boot",
               geom = "errorbar",
