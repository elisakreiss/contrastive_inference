...
ggplot(df_adj_rlvl, aes(x=obj_in_display, y=clicked, fill=obj_in_display)) +
  facet_wrap(vars(condition), nrow = 2) +
  stat_summary(fun.y = "mean", 
               geom = "bar") +
  stat_summary(fun.data = "mean_cl_boot",
...
df_adj_rlvl %>% 
  ...
  ggplot(., aes(x=obj_in_display, y=clicked, fill=trial_half)) +
    facet_wrap(vars(condition), nrow = 2) +
    stat_summary(fun.y = "mean", 
                 geom = "bar",
...
df_compare %>% 
  ...
  ggplot(., aes(x=obj_in_display, y=clicked, fill=timeStep_selec)) +
    facet_wrap(vars(condition), nrow = 2) +
    stat_summary(fun.y = "mean",
                 geom = "bar",
                 position = position_dodge(width = 0.9)) +
...
df_model_prior = df %>% 
  ...
  # select columns relevant for main trials
  select(target,comp,contrast,distractor,selectedItem_prior,selectedItem1,anon_worker_id,trial_number,condition) %>% 
  # encode which contexts have a contrast
  mutate(contrast_present = str_detect(condition,"p")) %>%
  # import typicality
  left_join(typ_data_short, by=c("target"="col_type")) %>% 
  ...
  mutate(clickedType_adj = case_when(
    ...
    lastSelectedItem_prior == target ~ "comp",
    TRUE ~ "other"
  )) %>% 
...
formula_priorc = click_to_target ~ c_prior_click + c_contrast*c_target_typ_bin*c_comp_typ_bin + (1+c_contrast*c_target_typ_bin|anon_worker_id) + (1+c_contrast*c_target_typ_bin*c_comp_typ_bin|target)

df_model_pos = df_adj %>%
  select(clickedType, target_typ, comp_typ, target_typ_bin, comp_typ_bin, contrast_present, anon_worker_id, trial_number, target, pos1, pos2, pos3, pos4) %>%
  distinct() %>%
  filter(clickedType == "target" | clickedType == "comp") %>%
  mutate(click_to_target = ifelse(clickedType == "target", 1, 0)) %>%
...
formula_pos = click_to_target ~ c_pos + c_contrast*c_target_typ_bin*c_comp_typ_bin + (1+c_contrast*c_target_typ_bin|anon_worker_id) + (1+c_contrast*c_target_typ_bin*c_comp_typ_bin|target)
...
summary(model_noprior)

model_prior = brm(
  formula = formula_priorc,
  data = df_model_prior,
  seed = 1702
)
...
plot_posterior_density_noprior = 
  model_noprior %>% 
  as_tibble() %>% 
...
plot_posterior_density_prior = 
  ...
  geom_segment(
    ...
    color = "firebrick",
    size = 3,
    data = fixef(model_prior) %>% as_tibble() %>%
...
plot_posterior_density_pos = 
  ...
  geom_segment(
    ...
    color = "firebrick",
    size = 3,
    data = fixef(model_pos) %>% as_tibble() %>%
...
ggsave(plot = plot_posterior_density_prior, filename = "graphs/posterior_density_prior.png",
       width = 15, height = 10)

ggsave(plot = plot_posterior_density_noprior, filename = "graphs/posterior_density_noprior.png",
       width = 15, height = 10)

ggsave(plot = plot_posterior_density_pos, filename = "graphs/posterior_density_pos.png",
       width = 15, height = 10)
```
...
plot_posterior_density_predprior = 
  ...
  geom_segment(
    ...
    color = "firebrick",
    size = 3,
    data = fixef(model_predprior) %>% as_tibble() %>%
      mutate(parameter = c("Intercept", "c_pos", "c_contrast", "c_target_typ_bin", "c_comp_typ_bin", "c_contrast:c_target_typ_bin", "c_contrast:c_comp_typ_bin", "c_target_typ_bin:c_comp_typ_bin", "c_contrast:c_target_typ_bin:c_comp_typ_bin")) %>%
      mutate(parameter = as.factor(parameter)) %>%
      mutate(parameter = factor(parameter, levels = c("Intercept", "c_pos", "c_contrast", "c_target_typ_bin", "c_comp_typ_bin", "c_contrast:c_target_typ_bin", "c_contrast:c_comp_typ_bin", "c_target_typ_bin:c_comp_typ_bin", "c_contrast:c_target_typ_bin:c_comp_typ_bin"))) %>%
      rename(low = Q2.5, high = Q97.5))

ggsave(plot = plot_posterior_density_predprior, filename = "graphs/prior_density.png",
       width = 15, height = 10)
