...
df_prior_wf_typ_toplot = df_prior_wf_typicality %>% 
  mutate(obj_typ=ifelse(obj_in_display_typ=="typical",1,0),
         clickedTypicality=case_when(
           clickedType=="target" ~ target_typ_bin,
           clickedType=="comp" ~ comp_typ_bin,
           clickedType=="contrast/distractor" ~ contrast_typ_bin,
           TRUE ~ distractor_typ_bin
         )) %>% 
  group_by(anon_worker_id, trial_number, clickedTypicality) %>%
  summarize(random_baseline=sum(obj_typ/n())) %>%
  mutate(bin_clicked_typ=ifelse(clickedTypicality=="typical",1,0)) %>% 
  ungroup() %>% 
  gather(measure, value, bin_clicked_typ, random_baseline) %>% 
  mutate_at(vars(measure),funs(factor(., levels = c("random_baseline", "bin_clicked_typ"))))

# prior_typ_baseline = df_prior_wf_typ_toplot %>% 
#   filter(measure=="random_baseline") %>% 
#   group_by(measure) %>% 
#   mutate(mean=mean(value))

ggplot(df_prior_wf_typ_toplot, aes(x=measure, y=value, fill=measure)) +
  stat_summary(fun.y = "mean", 
               geom = "bar",
               width = 0.7) +
  stat_summary(fun.data = "mean_cl_boot",
               ...
               size = .3,
               width = 0.2) +
  # geom_hline(yintercept = unique(prior_typ_baseline$mean)) +
  theme(legend.position = "none",
        axis.ticks.x = element_blank()) +
  scale_x_discrete(labels=c("random_baseline"="random",
                            "bin_clicked_typ"="clicked")) +
...
df_adj_typ_toplot = df_adj_typicality %>% 
  mutate(obj_typ=ifelse(obj_in_display_typ=="typical",1,0),
         clickedTypicality=case_when(
           clickedType=="target" ~ target_typ_bin,
           clickedType=="comp" ~ comp_typ_bin,
  ...
  group_by(anon_worker_id, trial_number, clickedTypicality, timeStep_selec) %>%
  summarize(random_baseline=sum(obj_typ/n())) %>%
  mutate(bin_clicked_typ=ifelse(clickedTypicality=="typical",1,0)) %>% 
  ungroup() %>% 
  mutate(mean_val=mean(random_baseline),
         mean_val2=mean(bin_clicked_typ)) %>% 
  gather(measure, value, bin_clicked_typ, random_baseline) %>% 
  mutate_at(vars(measure),
            funs(factor(., levels = c("random_baseline", "bin_clicked_typ"))))

ggplot(df_adj_typ_toplot, aes(x=measure, y=value, fill=measure, alpha=timeStep_selec)) +
  stat_summary(fun.y = "mean", 
               geom = "bar",
               width = 0.7,
...
```{r}
df_diff = df_prior_vs_adj_unique %>% 
  mutate(clicked_diff = ifelse(timeStep_selec == "selectedItem_prior", -1, 1)) %>% 
  mutate_at(vars(clickedType), funs(factor(., levels=c("target", "comp", "contrast/distractor", "distractor"))))

ggplot(df_diff, aes(x=clickedType, 
                    y=clicked_diff, 
                    fill=clickedType)) +
...
#   mutate(clicked_diff = case_when(
#     timeStep_selec == "selectedItem_prior" ~ clicked*(-1),
#     TRUE ~ clicked*1
#     )) %>% 
#   mutate_at(vars(clickedType), funs(factor(., levels=c("target", "comp", "contrast/distractor", "distractor"))))
# 
...
ggplot(df_prior_vs_adj, aes(x=obj_in_display_diff, 
                            y=clicked, 
                            fill=obj_in_display_diff, 
...
df_diff = df_prior_vs_adj %>% 
  group_by(timeStep_selec, obj_in_display, condition_lvl) %>% 
  summarize(group_mean =  mean(clicked)) %>% 
  select(timeStep_selec, obj_in_display, condition_lvl, group_mean) %>% 
  ungroup() %>% 
...
ggplot(df_diff, aes(x=obj_in_display, 
                    y=ratio_0)) +
  facet_wrap(~condition_lvl, scales = "free_x", nrow=2) +
...
df_switch %>% 
  ...
  mutate_at(vars(targcomp_combo), funs(factor(., levels=c("target_target", "target_comp", 
                                     "comp_comp", "comp_target")))) %>% 
  ...
  ggplot(., aes(x=targcomp_combo, y=selected_combo, fill=adap_switch)) +
    facet_wrap(~condition_lvl, nrow=2) +
    stat_summary(fun.y = "mean", 
                 geom = "bar") +
    stat_summary(fun.data = "mean_cl_boot",
                 geom = "errorbar",
...
# for binomial
f_prereg = click_to_target | trials(ntrials) ~ c_contrast*c_target_typ_bin*c_comp_typ_bin + (1+c_contrast*c_target_typ_bin|anon_worker_id) + (1+c_contrast*c_target_typ_bin*c_comp_typ_bin|target)

model_prereg = brm(
  formula = f_prereg,
  data = df_model_prereg,
  seed = 1702,
  family = 'binomial' # or bernoulli?
)
