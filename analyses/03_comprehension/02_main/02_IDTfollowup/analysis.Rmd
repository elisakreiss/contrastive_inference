...
df_clean = df_import %>% 
  ...
  filter(anon_worker_id!="221402122691845" & anon_worker_id!="883798243080367" & anon_worker_id!="894525647067310" & anon_worker_id!="785587200485586" & anon_worker_id!="730874762120224")
  # 67, 4012
  # 5) if hit was done multiple times by a worker
  # filter(anon_worker_id!="886492649205797")
  # 66, 3894

# number of participants before exclusion (156)
length(unique(df_import$anon_worker_id))
# number of participants after exclusion (148)
length(unique(df_clean$anon_worker_id))

# 74 participant for atypical condition remain
# 69 participant for atypical condition remain
...
plot=ggplot(df_prior_wf_contrast[df_prior_wf_contrast$contrast_present=="contrast",], aes(x=obj_in_display_diff, 
                                 y=clicked, 
                                 fill=obj_in_display_diff, 
                                 alpha=trial_half)) +
  facet_wrap(vars(anon_worker_id), scales = "free_x") +
  stat_summary(fun.y = "mean", 
               geom = "bar",
               width = 0.7,
               position = position_dodge(width=0.65)) +
...
# ggsave(plot, file="by_id.pdf", width=25, height=25)
```
...
ggplot(df_rt, aes(x=reorder(condition,rt_shortened), y=rt_shortened)) + 
...
# significant comp typicality, prev_click, pos(target pref), pos(equal pos), prev_click:pos(target_pref)
