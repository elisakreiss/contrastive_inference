...
df_import = read_csv(here("data","03_comprehension","02_main","0506_0304_noprevsel","data.csv")) %>%
  # exclude test
...
engl_spellings = c("english", "englis", "eng", "engilsh", "englsh", "en", "englsih")
...
df_clean = df_import %>% 
  ...
  filter(anon_worker_id!="904008850023353" & anon_worker_id!="644735035526807" & anon_worker_id!="595368156747411" & anon_worker_id!="541431996036045" & anon_worker_id!="540253318106427" & anon_worker_id!="530564507948315" & anon_worker_id!="410020360756582" & anon_worker_id!="307518565659996" & anon_worker_id!="055029769379152")
  # 67, 4012
...
```{r}
write_csv(df_adj, "emp_noprevsel_prior.csv")
```
...
df_adj %>% 
  ...
  ggplot(aes(x=obj_in_display_diff, y=clicked, fill=obj_in_display_diff, alpha=prior)) +
...
df_adj %>%
  ...
  ggplot(aes(x=obj_in_display, y=clicked, fill=obj_in_display, alpha=prior)) +
...
```{r switch, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
df_model_prevclick =  df %>% 
  ...
  filter(trial_type == "critical") %>% 
  # select columns relevant for main trials
  select(prior,target,comp,contrast,distractor,selectedItem1,
  ...
  select(click_to_target,c_contrast,c_target_typ_bin,c_comp_typ_bin,c_prior_cond,anon_worker_id,target,comp)

table(df_model_prevclick$prior,df_model_prevclick$click_to_target,df_model_prevclick$contrast_present,df_model_prevclick$target_typ_bin,df_model_prevclick$comp_typ_bin)
...
f_new1 = click_to_target ~ c_contrast*c_target_typ_bin*c_comp_typ_bin*c_prior_cond + (1+c_contrast*c_target_typ_bin*c_prior_cond|anon_worker_id) +  (1+c_contrast*c_comp_typ_bin*c_prior_cond|target) + (1+c_contrast*c_target_typ_bin*c_prior_cond|comp)

f_new1 = click_to_target ~ c_contrast*c_target_typ_bin*c_comp_typ_bin*c_prior_cond + (1+c_target_typ_bin*c_prior_cond|anon_worker_id) +  (1+c_comp_typ_bin*c_prior_cond|target) + (1+c_target_typ_bin*c_prior_cond|comp)

library(lme4)

m = glmer(f_new1, data=df_model_prevclick, family = "binomial")
