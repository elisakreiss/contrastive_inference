...
df_CI = df_adj %>%
  mutate(CI_condition = str_sub(condition,1,2)) %>% 
  mutate(CI_clicked = ifelse(contrast_present, clicked, clicked*(-1))) %>% 
  mutate_at(vars(CI_condition), funs(factor(., levels=c("at", "tt", "aa", "ta"))))

df_CI %>% 
  ggplot(., aes(x=obj_in_display, 
                            y=clicked, 
                            fill=obj_in_display,
                            alpha=contrast_present)) +
  facet_wrap(~CI_condition, scales = "free_x", nrow=1) +
  stat_summary(fun.y = "mean", 
               geom = "bar",
               width = 0.8,
               position = position_dodge(width = 0.8)) +
...
```{r rsa}
write_csv(df_adj, here("models","02_prodInfRSA","data","emp_compr_noprior_data.csv"))
```
...
# effect of contrast, target typicality and comp typicality
